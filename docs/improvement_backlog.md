...
| Release automation | Public OSS needs predictable artifacts. | GitHub Actions now runs lint, compile, docs link checks, release notes preview, tests, benchmark smoke, package build, artifact uploads, tag-triggered GitHub Release creation, source/wheel/checksum release attachments, and Sigstore keyless signing bundles; next add optional PyPI trusted publishing. |
...
bundles; and `geond install` previews or writes common MCP/editor config files.
A good next slice is optional PyPI trusted publishing or editor-extension
commands.
