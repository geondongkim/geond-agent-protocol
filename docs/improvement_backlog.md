...
| Improvement | Why | Candidate implementation |
| --- | --- | --- |
| Local agent dashboard | Humans and PM agents need to see which agent is doing what without reading raw MCP JSON. | `geond dashboard serve` now serves Mission Control tabs, horizontally expanding Agent Fleet lanes, session/message cards, reservations, handoff-board lanes, code-risk hot files, Changesets review lanes, lineage counts, Usage Evidence, and Activity Timeline; next add richer collaboration graph drilldowns. |
