...
| Antigravity documentation draft worker | Documentation and backlog slices can benefit from a second drafting voice without giving another agent commit authority. | Allow `/Users/geondongkim/.local/bin/agy --print` as a documentation-only draft worker for neutral docs/backlog text, then require the Codex controller to verify facts, scope, safety boundaries, and repository fit before editing, committing, or citing the draft as evidence. |
| Protocol/orchestrator package split follow-through | The same-repo namespace split now keeps controller policy under `geond_orchestrator` while preserving legacy wrappers, but the physical package split still needs stable release contracts and downstream migration time. | Stabilize the public storage/MCP/dashboard contracts consumed by the controller, release `geond-agent-protocol` with `geond orch` delegation, then move `src/geond_orchestrator/`, controller CLI tests, prompts, worker templates, and playbooks into a separate `geond-orchestrator` package. |
