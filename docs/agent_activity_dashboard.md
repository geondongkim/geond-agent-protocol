...
Timeline rows expose a bounded expandable detail pane with event ids, artifact
links, agent identity, occurrence time, compact metadata previews, and related
review context from loaded sessions, handoffs, changesets, and graph nodes.
Usage Evidence separates conversation, work, and validation evidence refs so
...
and additions/deletions for focused review. The Graph view shows the bounded
lineage nodes and edges behind the Relationships view. The same read models are
...
| 4. Hook adapters | Capture real agent lifecycle events. | Implemented CLI-backed Codex/Claude hook examples that record lifecycle and validation events without raw prompts, secrets, or full logs. |
