...
### 3.1 에이전트를 만들기보다 공유 컨텍스트 계층을 만든다

가장 큰 개선점은 프로젝트의 정체성을 명확히 하는 것이다. Geond는 새로운 코딩 에이전트가 아니라, 여러 역할의 AI 에이전트가 공유하는 컨텍스트 및 협업 조정 계층이다. 현재 구현은 개발 저장소와 에이전트 세션을 가장 잘 다루지만, 기획, 제품, 디자인, QA, 마케팅, 고객지원, 운영, 보안, 문서화, 배포, PM/오케스트레이션 에이전트도 같은 evidence model을 사용할 수 있어야 한다.

이 선택의 장점:

- Copilot, Codex-like CLI, Continue, OpenHands 같은 개발 도구뿐 아니라 문서/디자인/QA/마케팅/운영 도구를 경쟁자가 아니라 사용자로 삼을 수 있다.
...
일반 RAG는 질문과 문서 조각의 유사도를 찾는다. 저장소와 업무 산출물을 함께 다루는 에이전트에는 이것만으로 부족하다.
