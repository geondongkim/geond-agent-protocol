...
def test_dashboard_route_matching_and_index() -> None:
    ...
    assert match_workspace_route("/api/workspaces/abc-123/code-risk") == (
        "abc-123",
        "code-risk",
    )
    assert match_workspace_route("/api/workspaces/abc-123/changesets") == (
        "abc-123",
        "changesets",
    )
    ...
    assert match_workspace_route("/api/workspaces/abc-123/orchestration-plan") == (
        "abc-123",
        "orchestration-plan",
    )
    assert match_workspace_route("/api/workspaces/abc-123/orchestration-actions") == (
        "abc-123",
        "orchestration-actions",
    )
    assert match_workspace_route("/api/workspaces/abc-123/orchestration-action-queue") == (
        "abc-123",
