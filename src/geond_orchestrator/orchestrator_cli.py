...
def main(*, prog: str = "geond-orchestrator") -> None:
    ...
    worker_cmd = subparsers.add_parser(
        "worker",
        help="Delegate worker lease commands to the protocol CLI",
    )
    worker_cmd.add_argument(
        "worker_args",
        nargs=argparse.REMAINDER,
        help="Arguments passed through after `geond worker`",
    )

    ci_cmd = subparsers.add_parser("ci", help="Watch GitHub CI runs")
    ci_subparsers = ci_cmd.add_subparsers(dest="ci_command", required=True)
    ci_watch = ci_subparsers.add_parser("watch", help="Delegate to `gh run watch`")
    ci_watch.add_argument("run_id")
    ci_watch.add_argument("-R", "--repo", required=True)
    ci_watch.add_argument("--exit-status", action="store_true")
    ci_watch.add_argument("--compact", action="store_true")
    ci_watch.add_argument("-i", "--interval", type=int)
    ...
    if args.command == "worker":
        if not args.worker_args:
            parser.error("worker requires arguments passed to `geond worker`")
        run_geond_cli_alias(["worker", *args.worker_args])
        return
    if args.command == "ci":
        run_ci_watch(args)
        return
...
def run_geond_cli_alias(argv: list[str]) -> None:
    from geond.cli import main as geond_main
...
def run_ci_watch(args: argparse.Namespace) -> None:
    command = ["gh", "run", "watch", args.run_id, "--repo", args.repo]
    if args.exit_status:
